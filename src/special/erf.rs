...
#[inline]
fn erf_impl_f64(z: f64, inv: bool) -> f64 {
    use super::evaluate::polynomial_f64;

    if z < 0.0 {
        if !inv {
            return -erf_impl_f64(-z, false);
        }
        if z < -0.5 {
            return 2.0 - erf_impl_f64(-z, true);
        }
        return 1.0 + erf_impl_f64(-z, false);
    }
    ...
    let result = if z < 0.5 {
        if z < 1e-10 {
            ...
            z * 1.125 + z * polynomial_f64(z, ERF_IMPL_AN) / polynomial_f64(z, ERF_IMPL_AD)
        }
    } else if z < 110.0 {
        let (r, b) = if z < 0.75 {
            (
...
#[inline]
fn erf_inv_impl_f64(p: f64, q: f64, s: f64) -> f64 {
    use super::evaluate::polynomial_f64;

    let result = if p <= 0.5 {
        let g = p * (p + 10.0);
        let r = polynomial_f64(p, ERF_INV_IMPL_AN) / polynomial_f64(p, ERF_INV_IMPL_AD);
        g * 0.0891314744949340820313 + g * r
    } else if q >= 0.25 {
        let g = (-2.0 * q.ln()).sqrt();
        let xs = q - 0.25;
        let r = polynomial_f64(xs, ERF_INV_IMPL_BN) / polynomial_f64(xs, ERF_INV_IMPL_BD);
        g / (2.249481201171875 + r)
    } else {
        let x = (-q.ln()).sqrt();
        if x < 3.0 {
            let xs = x - 1.125;
            let r = polynomial_f64(xs, ERF_INV_IMPL_CN) / polynomial_f64(xs, ERF_INV_IMPL_CD);
            0.807220458984375 * x + r * x
        } else if x < 6.0 {
            let xs = x - 3.0;
            let r = polynomial_f64(xs, ERF_INV_IMPL_DN) / polynomial_f64(xs, ERF_INV_IMPL_DD);
            0.93995571136474609375 * x + r * x
        } else if x < 18.0 {
            let xs = x - 6.0;
            let r = polynomial_f64(xs, ERF_INV_IMPL_EN) / polynomial_f64(xs, ERF_INV_IMPL_ED);
            0.98362827301025390625 * x + r * x
        } else if x < 44.0 {
            let xs = x - 18.0;
            let r = polynomial_f64(xs, ERF_INV_IMPL_FN) / polynomial_f64(xs, ERF_INV_IMPL_FD);
            0.99714565277099609375 * x + r * x
        } else {
            let xs = x - 44.0;
            let r = polynomial_f64(xs, ERF_INV_IMPL_GN) / polynomial_f64(xs, ERF_INV_IMPL_GD);
            0.99941349029541015625 * x + r * x
        }
    };
    s * result
}
...
#[inline]
pub(crate) fn erfc<F: Float>(x: F) -> F {
    if x.is_nan() {
        F::nan()
        ...
        F::from(erf_impl_f64(x.to_f64().unwrap(), true)).unwrap()
    }
}

#[inline]
pub(crate) fn erfc_inv<F: Float>(x: F) -> F {
    if x <= F::zero() {
        F::infinity()
    } else if x >= F::from(2.0).unwrap() {
        F::neg_infinity()
    } else {
        let xf = x.to_f64().unwrap();
        let result = if xf > 1.0 {
            erf_inv_impl_f64(-1.0 + xf, 2.0 - xf, -1.0)
        } else {
            erf_inv_impl_f64(1.0 - xf, xf, 1.0)
        };
