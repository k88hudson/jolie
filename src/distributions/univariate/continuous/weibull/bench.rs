//! Benchmarks for `Weibull`, comparing each trait method against `rand_distr`
...
//!
//! jolie and `statrs` take `(shape, scale)`; `rand_distr` takes `(scale, shape)`.
...
use rand_distr::Weibull as RandWeibull;
...
use statrs::distribution::{Continuous, ContinuousCDF, Weibull as StatrsWeibull};
...
fn jolie_dist() -> Weibull<f64> {
    Weibull::new(SHAPE, SCALE).unwrap()
}

fn statrs_dist() -> StatrsWeibull {
    StatrsWeibull::new(SHAPE, SCALE).unwrap()
}

fn sample(c: &mut Criterion) {
    let jd = jolie_dist();
    let sd = statrs_dist();
    let rd = RandWeibull::new(SCALE, SHAPE).unwrap();
    let mut rng = StdRng::seed_from_u64(0xC0FFEE);
    let mut rng8 = StdRng08::seed_from_u64(0xC0FFEE);

    let mut g = c.benchmark_group("weibull/sample");
...
fn density(c: &mut Criterion) {
    ...
    let mut g = c.benchmark_group("weibull/pdf");
    g.bench_function("jolie", |b| {
        b.iter(|| {
            let x = black_box(X);
            black_box(jd.pdf(&x))
    ...
    let mut g = c.benchmark_group("weibull/log_pdf");
    g.bench_function("jolie", |b| {
        b.iter(|| {
            let x = black_box(X);
...
fn cumulative(c: &mut Criterion) {
    ...
    let mut g = c.benchmark_group("weibull/cdf");
    ...
    let mut g = c.benchmark_group("weibull/ccdf");
    ...
    let mut g = c.benchmark_group("weibull/inverse_cdf");
    g.bench_function("jolie", |b| {
    ...
    let mut g = c.benchmark_group("weibull/log_cdf");
...
fn moments(c: &mut Criterion) {
    ...
    let mut g = c.benchmark_group("weibull/kurtosis");
