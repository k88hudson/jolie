//! Benchmarks for `Normal`, comparing each trait method against `rand_distr`
...
fn jolie_dist() -> Normal<f64> {
    Normal::new(MEAN, STD).unwrap()
}

fn statrs_dist() -> StatrsNormal {
    StatrsNormal::new(MEAN, STD).unwrap()
}

fn sample(c: &mut Criterion) {
    let jd = jolie_dist();
    let sd = statrs_dist();
    let rd = RandNormal::new(MEAN, STD).unwrap();
    let mut rng = StdRng::seed_from_u64(0xC0FFEE);
    let mut rng8 = StdRng08::seed_from_u64(0xC0FFEE);

    let mut g = c.benchmark_group("normal/sample");
...
fn density(c: &mut Criterion) {
    ...
    let mut g = c.benchmark_group("normal/pdf");
    g.bench_function("jolie", |b| {
        b.iter(|| {
            let x = black_box(X);
            black_box(jd.pdf(&x))
    ...
    let mut g = c.benchmark_group("normal/log_pdf");
    g.bench_function("jolie", |b| {
        b.iter(|| {
            let x = black_box(X);
...
fn cumulative(c: &mut Criterion) {
    ...
    let mut g = c.benchmark_group("normal/cdf");
    ...
    let mut g = c.benchmark_group("normal/ccdf");
    ...
    let mut g = c.benchmark_group("normal/inverse_cdf");
    g.bench_function("jolie", |b| {
    ...
    let mut g = c.benchmark_group("normal/log_cdf");
...
fn moments(c: &mut Criterion) {
    ...
    let mut g = c.benchmark_group("normal/kurtosis");
